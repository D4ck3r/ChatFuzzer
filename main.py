...
async def main():
    ...
    await utils.init_seed_temple_queue()
    ...
    gpt_task = asyncio.create_task(task(utils.gpt_chat_queue)) #gpt task generate seed template
                                                               # gpt task generate seed template
    manage_sessions = asyncio.create_task(monitor.manage_sessions())
    mutator_task = asyncio.create_task(mutator.task(utils.seed_temple_queue))
